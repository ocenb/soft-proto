...
def update_user_token(db: Session, user_id: int, token: str = None):
...
def populate_from_csv(db: Session, file_path: str = "students.csv"):
    with open(file_path, "r", encoding="utf-8") as f:
        ...
        for row in reader:
            ...
            grade = models.Grade(
                student_id=student_cache[key], subject=subject, score=score
            )
            db.add(grade)
        db.commit()
...
def get_students_by_course_and_score(db: Session, course: str, max_score: int = 30):
    return (
        db.query(models.Student)
        .join(models.Grade)
        .filter(models.Grade.subject == course, models.Grade.score < max_score)
        .all()
    )
...
def get_average_score_by_faculty(db: Session):
    results = (
        db.query(
            models.Student.faculty, func.avg(models.Grade.score).label("average_score")
        )
        .join(models.Grade)
        .group_by(models.Student.faculty)
        .all()
    )
